...
class AScan(torch.autograd.Function): # (b c l p)
    ...
    def forward(ctx, g, x):
        mode = "pointwise" if g.device.type == "cuda" else "generic"
        _, x_scan = associative_scan(AScan.scan_op, (g, x), dim=2, combine_mode=mode)
    ...
    def backward(ctx, grad):
        ...
        g = F.pad(g, (0, 0, -1, 1))
        mode = "pointwise" if g.device.type == "cuda" else "generic"
        _, x_grad = associative_scan(AScan.scan_op, (g, grad), dim=2, reverse=True, combine_mode=mode)
